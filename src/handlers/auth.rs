...
pub struct RegisterRequest {
    ...
    pub phone: String,
    pub password: String,
    // Para clientes, não será permitido definir o role manualmente.
}
...
/// Endpoint de registro para clientes: cria um novo usuário com role "client"
...
/// Endpoint de login: valida telefone e senha e gera o código SMS para verificação.
pub async fn login(
    Extension(pool): Extension<Pool>,
    ...
    // Busca o usuário pelo telefone e garante que o role seja "client"
    let user: User = users
    ...
    // Gera e "envia" o código SMS (simulado)
...
/// Endpoint de verificação: valida o código SMS e retorna um token JWT se válido.
pub async fn verify(
    Extension(pool): Extension<Pool>,
    ...
    // Validação simples: se o código tiver 6 dígitos, consideramos-o válido.
...
/// Endpoint de troca de senha: verifica a senha atual e atualiza com a nova senha.
...
//
// RECUPERAÇÃO DE SENHA (ESQUECI A SENHA / RESET)
//
...
pub struct ForgotPasswordResponse {
    pub message: String,
    // Apenas para teste, retorna o token; em produção, não retorne o token na resposta.
    pub token: Option<String>,
}

/// Claims para reset de senha (válido por 5 minutos)
#[derive(Serialize, Deserialize)]
pub struct ResetClaims {
    pub sub: String,   // ID do usuário
    pub exp: usize,    // Expiração (5 minutos)
    pub reset: bool,   // Flag indicando que o token é para reset de senha
}
...
pub async fn forgot_password(
    ...
    // Simula o envio do token via SMS (aqui, apenas imprime no console).
