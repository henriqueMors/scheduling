...
// Estrutura das claims esperadas no token JWT (usado para extrair o ID do admin master)
...
pub async fn add_admin_handler(
    // Extração do header Authorization usando TypedHeader
    TypedHeader(Authorization(bearer)): TypedHeader<Authorization<Bearer>>,
    Extension(pool): Extension<Pool>,
    Json(payload): Json<AddAdminRequest>,
) -> Result<Json<AdminResponse>, (StatusCode, String)> {
    // Decodifica o token JWT para obter as claims (incluindo o ID do admin master)
